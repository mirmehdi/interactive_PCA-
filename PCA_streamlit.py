...
if uploaded_file is not None:
    ...
    num_components = st.slider("Select number of principal components", 
                               min_value=1, 
                               max_value=len(cumulative_explained_variance), 
                               value=int(np.argmax(cumulative_explained_variance >= 0.90) + 1))
    st.write(f"Number of components selected: {num_components}")

    # Proceed button to continue with model training and evaluation
    if st.button("Proceed with Model Training and Evaluation"):
        ...
        # Train and evaluate Logistic Regression model with PCA features
        ...
        # Train and evaluate Logistic Regression model with all features
